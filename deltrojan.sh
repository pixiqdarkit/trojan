...
NUMBER_OF_CLIENTS=$(grep -c -E "^#tcp " "/usr/local/etc/xray/config.json")
	...
	grep -E "^#tcp " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 2-3 | nl -s ') '
...
user=$(grep -E "^#tcp " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^#tcp " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
